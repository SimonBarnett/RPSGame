...
(function (global) {
  'use strict';
  const BUILD = { n: 151, gen: 2091, games: 16122, at: "2026-09-16 10:32Z", sha: "15310fe" };
  /**
  ...
  const THRUST = 0.22;
  const MASS = { ROCK: 1.35, SCISSORS: 0.88, PAPER: 0.95 };
  ...
  function huntHeading(p, prey, look, fearD) {
    ...
    if (p.type === 'SCISSORS') return interceptHeading(p, prey, 'lead');
  ...
  function createSim(opts) {
    ...
    function safestPrey(p, preyT, fearT) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        let s = fd - 0.45 * pd;
        const iso = p.type === 'PAPER' ? 8 : (p.type === 'SCISSORS' ? 20 : 5.5);
    ...
    function think(p, c) {
      ...
      if (p.type === 'PAPER' && fearN > 0) {
        ...
        if (fd < p.size * 32 || !prey || fd <= prey.d) prey = null;
      }
      ...
      if (fearN > 0 && preyN <= 1 && prey && prey.d < p.size * 6) {
        ...
        const scissorsHunt = p.type === 'SCISSORS' && prey;
        if (fearClose && !paperHunt && !rockChase && !scissorsHunt) {
          mode = 'evade';
          want = safeH;
          p._locked = null;
        } else if (prey) {
      ...
      // Scissors: kite Rocks inside 20× so converted Paper is not eaten next to Rocks.
      if (p.type === 'SCISSORS' && fear && fear.obj && fear.d < p.size * 20) {
        want = safeH;
    ...
    function paperNeverIntoScissors(p, want, prey) {
      ...
      const clipsXY = function (h, ox, oy, reach, rad) {
        const ux = Math.sin(h), uy = -Math.cos(h);
        const vx = ox - p.x, vy = oy - p.y;
