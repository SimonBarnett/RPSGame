...
(function (global) {
  'use strict';
  const BUILD = { n: 134, gen: 2017, games: 15662, at: "2026-09-15 21:05Z", sha: "2a9968f" };
  /**
  ...
  function createSim(opts) {
    ...
    function safestPrey(p, preyT, fearT) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        let s = fd - 0.45 * pd;
        const iso = p.type === 'PAPER' ? 8 : (p.type === 'SCISSORS' ? 16 : 5.5);
    ...
    function think(p, c) {
      ...
      if (p.type === 'SCISSORS' && fear && fear.obj && fear.d < p.size * 16) {
        want = safeH;
        want = noCloseOn(p, want, fear.obj, fear.d, 16);
      }
      if (want != null) {
