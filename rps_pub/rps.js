...
(function (global) {
  'use strict';
  const BUILD = { n: 135, gen: 2021, games: 15696, at: "2026-09-15 22:16Z", sha: "8977a60" };
  /**
  ...
  function createSim(opts) {
    ...
    function safestPrey(p, preyT, fearT) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        let s = fd - 0.45 * pd;
        const iso = p.type === 'PAPER' ? 8 : (p.type === 'SCISSORS' ? 18 : 5.5);
    ...
    function think(p, c) {
      ...
      if (p.type === 'SCISSORS' && fear && fear.obj && fear.d < p.size * 18) {
        want = safeH;
        want = noCloseOn(p, want, fear.obj, fear.d, 18);
      }
      if (want != null) {
